...
def barra_menu(root): #Sección para crear la barra de menú
    barra_menu = tk.Menu(root)
    root.config(menu = barra_menu, width = 300, height = 300)

    menu_inicio = tk.Menu(barra_menu, tearoff = 0) #Elimina los espacios en los menús

    barra_menu.add_cascade(label = 'Inicio', menu = menu_inicio) #Crea el menú de inicio en la barra de menú
    menu_inicio.add_command(label = 'Crear catálogo de películas en DB', command=crear_tabla)
    menu_inicio.add_command(label = 'Eliminar catálogo de películas en DB', command=borrar_tabla)
...
class Frame(tk.Frame):
    ...
    def tabla_peliculas(self):
        #Recuperar la lista de películas
        ...
        #Scrollbar si la lista excede 10 registros
